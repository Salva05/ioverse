...
const Temperature = () => {
  ...
  const DEFAULT_VALUE = () => assistant?.temperature || 1.0;

  // Hold the firstly computed top_p
  const historyValueRef = useRef(null);
  useEffect(() => {
    if (historyValueRef.current === null && assistant?.temperature !== undefined) {
      historyValueRef.current = assistant.temperature;
    }
  }, [assistant]);
